...
import { createUser } from '../../../../store/consumptionProfile';

const RegisterUserConsumptionRole = () => {
  const dispatch = useDispatch();
  const { users } = useSelector((state) => state.consumptionProfile);
  ...
  const renderUsersOptions = () => {
    return users.users.map((user) => (
      <option value={user.user} key={user.id}>
        {user.user}
      </option>
    ));
  };
...
export default RegisterUserConsumptionRole;
