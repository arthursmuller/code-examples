...
import { closeSubgroupToaster } from '../../../store/generalConfig';
import ListDrillDown from './ListDrillDown';

const SubgroupCard = () => {
  const intl = useIntl();
  const subgroupConfig = useSelector((state) => state.generalConfig);
  const dispatch = useDispatch();
  const [trColor, setTrColor] = useState(-1);
  const [open, setOpen] = useState(false);

  const { showToasterSubgroupCreate, showToasterSubgroupEdit } = subgroupConfig;
  ...
  const columns = React.useMemo(
    () => [
      ...
      {
        Header: intl.formatMessage({ id: 'global.subgroup' }),
        accessor: 'subgroup',
      },
      {
  ...
  return (
    <>
      <Toaster
        w="100%"
        mt="3%"
        open={showToasterSubgroupCreate || showToasterSubgroupEdit}
        onClose={() => dispatch(closeSubgroupToaster())}
      >
        {showToasterSubgroupCreate && (
          <FormattedMessage id="general_config.subgroup.toaster_create" />
        )}
        {showToasterSubgroupEdit && (
          <FormattedMessage id="general_config.subgroup.toaster_edit" />
        )}
      </Toaster>
      <Box
        d="flex"
      ...
      >
        <Box w="376px" ml="1.5%">
          <Input
            inputProps={{
              placeholder: intl.formatMessage({ id: 'global.select_subgroup' }),
        ...
        <Box alignSelf="center">
          ...
          <Button colorScheme="blue" h="45px" w="176px">
            <Link to="/general-config-subgroup-register">
                        ...
                        <Menu placement="top" onClose={() => changeColor(-1)}>
                          ...
                          <MenuList>
                            <MenuItem fontSize="sm">
                              <Link to="/general-config-subgroup-edit">
