...
import { closeUserToaster } from '../../../store/generalConfig';
import ListDrillDown from './ListDrillDown';

const UsersCard = () => {
  const intl = useIntl();
  const userConfig = useSelector((state) => state.generalConfig);
  const dispatch = useDispatch();
  const [trColor, setTrColor] = useState(-1);
  const [open, setOpen] = useState(false);

  const { showToasterUserCreate, showToasterUserEdit } = userConfig;
  ...
  return (
    <>
      <Toaster
        w="100%"
        mt="3%"
        open={showToasterUserCreate || showToasterUserEdit}
        onClose={() => dispatch(closeUserToaster())}
      >
        {showToasterUserCreate &&
      ...
      >
        <Box w="376px" ml="1.5%">
          <Input
            inputProps={{
              placeholder: intl.formatMessage({ id: 'global.select_user' }),
        ...
        <Box alignSelf="center">
          ...
          <Button colorScheme="blue" h="45px" w="176px">
            <Link to="/general-config-users-register">
                        ...
                        <Menu placement="top" onClose={() => changeColor(-1)}>
                          ...
                          <MenuList>
                            <MenuItem fontSize="sm">
                              <Link to="/general-config-users-edit">
