...
import { closeUsersToaster, updateUsersCategory } from '../../../store/site';

const UsersCard = () => {
  const dispatch = useDispatch();
  const { block_categories } = useSelector((state) => state.site);
  const { users } = block_categories;
  ...
  const submit = () => {
    dispatch(updateUsersCategory(categories));
  };
  ...
  return (
    <>
      <Toaster
        ...
        onClose={() => dispatch(closeUsersToaster())}
      >
