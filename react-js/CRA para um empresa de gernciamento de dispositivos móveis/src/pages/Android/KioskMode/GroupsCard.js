...
import { kioskUpdateGroup, kioskCloseGroupToaster } from '../../../store/site';

const GroupsCard = () => {
  const intl = useIntl();
  const dispatch = useDispatch();
  const { kiosk_mode } = useSelector((state) => state.site);
  const { showToaster: toaster, groups } = kiosk_mode;
  const [kioskActions, setKioskActions] = useState(groups);
  ...
  const submit = () => {
    dispatch(kioskUpdateGroup(kioskActions));
  };
  ...
  return (
    <>
      <Toaster
        ...
        onClose={() => dispatch(kioskCloseGroupToaster())}
      >
        ...
        <Box d="flex" flexDirection="column">
          <FormControl w="520px">
            <FormLabel fontSize="sm" color="gray.300">
              <FormattedMessage id="sites.block.group.form_label" />
            </FormLabel>
