...
import { kioskUpdateUsers, kioskCloseUsersToaster } from '../../../store/site';

const UsersCard = () => {
  const intl = useIntl();
  const dispatch = useDispatch();
  const { kiosk_mode } = useSelector((state) => state.site);
  const { showToaster: toaster, users } = kiosk_mode;
  const [kioskActions, setKioskActions] = useState(users);
  ...
  const submit = () => {
    dispatch(kioskUpdateUsers(kioskActions));
  };
  ...
  return (
    <>
      <Toaster
        ...
        ml="-1%"
        mt="2%"
        open={users.showToaster}
        onClose={() => dispatch(kioskCloseUsersToaster())}
      >
        ...
        <Box d="flex" flexDirection="column">
          <FormControl w="520px">
            <FormLabel fontSize="sm" color="gray.300">
              <FormattedMessage id="sites.block.user.form_label" />
            </FormLabel>
