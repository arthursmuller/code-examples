...
const GeneralCard = () => {
  ...
  const { showToaster: toaster, general } = kiosk_mode;
  const [kioskActions, setKioskActions] = useState(general);
  ...
  const submit = () => {
    dispatch(kioskUpdateGeneral(kioskActions));
  };
  ...
  const [selectedTab, setSelectedTab] = useState('disabled');
  ...
  return (
    <>
      <Toaster
        ...
        onClose={() => dispatch(kioskCloseGeneralToaster())}
      >
